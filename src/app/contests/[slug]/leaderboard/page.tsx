...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    if (initial && entries.length === 0) setEntries(initial);
  ...
  const problems = contest?.contest_problems ?? [];
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-8 flex-wrap">
          <Link
            href={`/contests/${slug}`}
            className="p-2 text-slate-500 hover:text-slate-200 hover:bg-surface-100 rounded-lg transition-colors"
          >
        ...
        {/* Full table with per-problem columns */}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-300 text-xs text-slate-500">
                ...
                {problems.map((cp, i) => (
                  <th key={cp.id} className="text-center px-3 py-3 w-20">
                    <div className="font-bold text-slate-400">{String.fromCharCode(65 + i)}</div>
                    <div className="text-[10px] text-slate-600">{cp.score}pt</div>
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-surface-300">
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-surface-300 flex items-center justify-center text-xs font-bold text-slate-300 shrink-0">
                        {(e.display_name || e.username)[0].toUpperCase()}
                      </div>
                      <div>
                        <div className="font-medium text-slate-200 text-sm">{e.username}</div>
                        {e.display_name && (
                          <div className="text-xs text-slate-500">{e.display_name}</div>
                        )}
                      </div>
                      {e.is_disqualified && (
                        <span className="text-xs text-red-400 bg-red-500/10 px-1.5 py-0.5 rounded border border-red-500/20 ml-1">DQ</span>
                      )}
                    </div>
                  </td>

                  {/* Per-problem cells — green if solved, red if attempted, dash if untouched */}
                  {problems.map((cp) => {
                    const stat = e.problem_stats?.[String(cp.id)];
                    if (!stat) return (
                      <td key={cp.id} className="px-3 py-3 text-center text-slate-700">—</td>
                    );
                    if (stat.solved) return (
                      <td key={cp.id} className="px-3 py-3 text-center">
                        ...
                        <div className="text-[10px] text-emerald-600">{stat.solve_time}m</div>
                      </td>
                    );
                    return (
                      <td key={cp.id} className="px-3 py-3 text-center">
                        <div className="text-red-400 text-xs">-{stat.wrong_attempts}</div>
                      </td>
                    );
                  })}
              ...
              {entries.length === 0 && !isLoading && (
                <tr>
                  <td colSpan={3 + problems.length} className="py-12 text-center text-slate-500 text-sm">
